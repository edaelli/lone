...
from lone.system import SysPci, SysPciDevice

import logging
logger = logging.getLogger('linux_pci')
...
class LinuxSysPciDevice(SysPciDevice):
    ...
    def _bind_to_driver(self, driver):
        ...
        if os.path.exists(path):
            ...
            logger.warning('Could not bind to driver: {}'.format(driver))
    ...
    def _create_device(self):
        ...
        if not os.path.exists(device_path):
            device_path = None
            logger.error('vfio device {} was not created'.format(self.iommu_group))

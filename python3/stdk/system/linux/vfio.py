...
from stdk.system import SysPciUserspace, SysPciUserspaceDevice
from stdk.nvme.spec.registers.pcie_regs import PCIeRegistersIndirect
...
class SysVfioIfc(SysPciUserspaceDevice):
    ...
    def nvme_regs(self):
        nvme_mmap = mmap.mmap(self.device_fd,
                              length=self.bars[0]['size'],
                              offset=self.bars[0]['offset'])
        return NVMeRegisters.from_buffer(nvme_mmap)
        ...
        for device in context.list_devices():
            ...
            if (device.get('SUBSYSTEM') == 'pci' and
                ...
                if not mounted:
                    ...
                    print('WARNING: Device {} ({}) is not available because it is mounted!'.format(
                        dev_name, slot))
